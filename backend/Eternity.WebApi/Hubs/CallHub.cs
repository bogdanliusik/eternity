    ...
    ILogger<CallHub> logger) : Hub
{
    ...
    public async Task JoinCall(Guid callId) {
        ...
        if (callConnectionTracker.IsSessionInAnyCall(userId, sessionId)) {
            if (callConnectionTracker.IsSessionInCall(callId, userId, sessionId)) {
                ...
                await Clients.Caller.SendAsync(
                    "CallError",
                    "You are already in another active call from this session."
                );
                return;
            }
        }
        ...
        if (!result.Succeeded) {
            var error = result.Errors.Count > 0 ? result.Errors[0] : "Failed to join call.";
        ...
        var participant = callDto.Participants.Find(p => p.UserId == userId);
        if (participant != null) {
            await Clients.OthersInGroup(callGroup)
                .SendAsync(
                    "ParticipantJoined",
                    new {
                        callId,
        ...
        foreach (var (peerUserId, peerId) in existingPeers) {
            await Clients.Caller.SendAsync("PeerIdRegistered", new { userId = peerUserId, peerId });
        }
    ...
    public async Task NotifyMediaStateChanged(Guid callId, bool audioEnabled, bool videoEnabled) {
        ...
        var callGroup = CallConnectionTracker.GetCallGroup(callId);
        await Clients.OthersInGroup(callGroup)
            .SendAsync("MediaStateChanged",
                new { userId = currentUser.Id, audioEnabled, videoEnabled });
    }
    ...
    public async Task LeaveCall(Guid callId) {
        ...
        if (!callConnectionTracker.IsUserInCall(callId, userId)) {
            ...
            if (!result.Succeeded) {
                var error = result.Errors.Count > 0 ? result.Errors[0] : "Failed to leave call.";
                await Clients.Caller.SendAsync("CallError", error);
                return;
            }
            await Clients.OthersInGroup(callGroup).SendAsync("ParticipantLeft", new { callId, userId });
            if (result.Data.CallEnded) {
                await Clients.Group(callGroup)
                    .SendAsync("CallEnded", new { callId, reason = "All participants left." });
            }
        }
    ...
    public async Task SendSignal(Guid callId, Guid targetUserId, object signal) {
        ...
        if (!usersInCall.Contains(targetUserId)) {
            return;
        }
        var targetConnectionIds = callConnectionTracker.GetConnectionIdsForUser(callId, targetUserId);
        var sendTasks = targetConnectionIds.Select(connId =>
            Clients.Client(connId).SendAsync("SignalReceived", new { fromUserId = currentUser.Id, signal })
        );

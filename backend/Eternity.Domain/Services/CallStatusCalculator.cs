...
public static class CallStatusCalculator
{
    ...
    public static CallStatus Calculate(IReadOnlyList<CallParticipant> participants, Guid initiatorId) {
        ...
        var anyInviteeEverJoined = invitees.Any(p => p.Status == ParticipantStatus.Left || p.JoinedAt != null);
        if (anyInviteeEverJoined) {
            return CallStatus.Completed;
        }
        var allInviteesDeclined = invitees.Count > 0 && invitees.All(p => p.Status == ParticipantStatus.Declined);
        ...
        var someDeclinedSomeMissed = invitees.Any(p => p.Status == ParticipantStatus.Declined) &&
                                     invitees.Any(p => p.Status is ParticipantStatus.Ringing or ParticipantStatus.Missed
                                     );
